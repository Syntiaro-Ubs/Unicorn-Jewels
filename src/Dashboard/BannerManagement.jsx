...
export default function BannerManagement() {
  ...
  const handleDelete = async (pageKey) => {
    if (
      !window.confirm(
        `Are you sure you want to delete the banner for "${pageKey}"?`,
      )
    )
      return;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1 font-medium">
            Control banners across all website pages and collections.
          </p>
        </div>
              ...
              {filteredBanners.map((banner) => (
                ...
                >
                  ...
                  <td className="px-8 py-6">
                    ...
                    <p className="text-slate-400 text-xs mt-1 truncate max-w-[200px]">
                      {banner.subtitle}
                    </p>
                  </td>
                  <td className="px-8 py-6">
            ...
            >
              <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h3 className="text-xl font-bold text-slate-800">
                  {editingBanner
                    ? `Edit Banner: ${editingBanner.page_key}`
                    : modalStep === 1
                      ? "Select Page"
                      : `Configure: ${formData.page_key}`}
                </h3>
                <button
                  onClick={() => setIsModalOpen(false)}
              ...
              {modalStep === 1 ? (
                <div className="flex-1 overflow-y-auto p-8 space-y-8">
                  <p className="text-sm text-slate-500 font-medium -mt-2">
                    Choose the page you want to create a banner for.
                  </p>
