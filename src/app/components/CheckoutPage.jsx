      ...
      <div className="w-full lg:w-[45%] xl:w-[40%] bg-[#fafafa] border-l border-gray-100 px-6 py-12 md:px-12 md:py-16">
        <motion.div initial={{
          ...
          {items.length === 0 ? <p className="text-sm text-gray-500 tracking-wider">Your bag is empty.</p> : <div className="space-y-6 mb-8 max-h-[40vh] overflow-y-auto pr-2" style={{
            ...
            {items.map(item => <div key={`${item.id}-${item.selectedSize || ''}-${item.selectedWeight || ''}`} className="flex gap-5">
              ...
              <div className="flex-1 flex flex-col justify-center">
                ...
                <div className="flex gap-2.5 mt-1">
                  {item.selectedSize && <p className="text-[8px] text-blue-600 font-bold tracking-wider uppercase">Size: {item.selectedSize}</p>}
                  {item.selectedWeight && <p className="text-[8px] text-emerald-600 font-bold tracking-wider uppercase">Weight: {parseFloat(item.selectedWeight).toFixed(2)}g</p>}
                </div>
